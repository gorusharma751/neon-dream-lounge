...
CREATE POLICY "Users can reserve available slots" ON public.time_slots FOR UPDATE USING ((status = 'available'::text) OR ((status = 'reserved'::text) AND (reserved_by = auth.uid())));
