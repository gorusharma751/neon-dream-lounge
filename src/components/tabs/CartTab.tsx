...
import { Trash2, Minus, Plus, ShoppingBag, CalendarDays, Loader2, CheckCircle } from 'lucide-react';
  ...
  return (
    <div className="px-4 pb-4">
      ...
      <div className="flex gap-2 mb-4">
        ...
        >
          <ShoppingBag size={14} className="inline mr-1" /> Cart ({items.length})
        </button>
        <button
        ...
        >
          <CalendarDays size={14} className="inline mr-1" /> History ({bookings.length})
        </button>
      </div>
      ...
      {!showHistory ? (
        ...
        <div className="space-y-4">
          ...
          {upcomingBookings.length > 0 && (
            <div>
              <h3 className="font-orbitron text-xs font-bold text-neon-green mb-2">UPCOMING</h3>
              {upcomingBookings.map(b => (
                <div key={b.id} className="glass rounded-lg p-3 mb-2 border border-neon-green/20">
                  ...
                  <p className="text-[10px] text-muted-foreground mt-1">ID: {b.id.slice(0, 8)}</p>
                </div>
              ))}
            </div>
          )}
          ...
          {pastBookings.length > 0 && (
            <div>
              <h3 className="font-orbitron text-xs font-bold text-muted-foreground mb-2">PAST</h3>
              {pastBookings.map(b => (
